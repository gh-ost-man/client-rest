      ...
      <table class="table custom-table mt-5">
        ...
        <tbody class="border border-dark">
          <tr class="text-center">
            ...
            <th class="border border-dark" scope="col">
             100
            </th>
            <th class="border border-dark" scope="col"></th>
            <th class="border border-dark" scope="col"></th>
          ...
          <template v-if="reports">
            <tr class="text-center" v-for="r in reports" :key="r.id">
              ...
              <th
                ...
                :key="ak.idQ"
                scope="row"
                :class="r.questionUnits
                      .find(
                        (x) => x.questionId === ak.idQ
                      )
                      ?.currentKeys.split(',')
                      .sort()
                      .join('') === ak.charKey? 'is-correct':'is-incorrect'"
              ...
              <td class="border border-dark">{{ new Date(r.reportDate).toLocaleDateString() }}</td>
            </tr>
          </template>
        </tbody>
      </table>
